...
exports.login = async (req,res, next)=>{
    ...
    if(!User||!await User.correctPassword(password, User.password)) return res.status(401).json({status: 'wrong details'});
    cookieStore(User, 200, res)}catch(err){res.status(400).json({status:'weird!', 
        message:err.message})}
}
